...
void layoutControls(HWND hwnd) {
    ...
    // 计算垂直位置
    int displayY = MARGIN;
    int buttonsY = height - MARGIN - BUTTON_HEIGHT;  // 按钮靠下对齐
...
JNIEXPORT void JNICALL Java_Counter_initializeNative
  (JNIEnv*, jclass) {
    ...
    wc.hCursor = LoadCursor(nullptr, IDC_ARROW);  // 添加标准鼠标指针
    wc.hbrBackground = (HBRUSH)(COLOR_WINDOW + 1);  // 添加标准窗口背景
    RegisterClassW(&wc);
}

JNIEXPORT jlong JNICALL Java_Counter_createWindow
  (JNIEnv*, jobject) {
...
JNIEXPORT void JNICALL Java_Counter_processMessages
  (JNIEnv*, jobject) {
    MSG msg;
